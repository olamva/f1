...
const race = (round: number, name: string): Upcoming => ({ round, name, kind: "race" });
const sprint = (round: number, name: string): Upcoming => ({ round, name, kind: "sprint" });
...
test("2025 finale: Norris had not clinched, Russell was out", () => {
  assert.equal(hasClinched(beforeAbuDhabi2025, abuDhabi, "norris", "drivers"), false);
  assert.equal(isEliminated(beforeAbuDhabi2025, abuDhabi, "russell", "drivers"), true);
  assert.equal(isEliminated(beforeAbuDhabi2025, abuDhabi, "piastri", "drivers"), false);
});

test("2025 finale matrix: P3 behind a Verstappen win was enough, P4 was not", () => {
  const m = clinchMatrix(beforeAbuDhabi2025, abuDhabi, "norris", "max_verstappen");
...
test("2025 finale simulated with the real result crowns Norris", () => {
  const picks = [{ max_verstappen: [1], piastri: [2], norris: [3], russell: [5] }];
...
test("2025 Qatar weekend: Norris could clinch at the Qatar race at the earliest", () => {
  ...
  const events = [sprint(23, "Qatar sprint"), race(23, "Qatar"), race(24, "Abu Dhabi")];
...
test("2025 constructors: McLaren clinched at Singapore with the real result", () => {
  assert.deepEqual(earliestClinch(afterBaku2025, restOf2025, "mclaren", "constructors"), {
    state: "possible",
    index: 0,
  });
  const picks = [{ mercedes: [1, 5], red_bull: [2], mclaren: [3, 4], ferrari: [6, 8] }];
...
test("2025 constructors: McLaren's real 27 points met the need against Mercedes' 35", () => {
  const needs = constructorNeeds(afterBaku2025, restOf2025, "mclaren", "mercedes");
