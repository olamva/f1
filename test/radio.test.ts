...
const word = (word: string, start: number) => ({ word, start, end: start + 0.2 });
...
test("radio text stays whole when no voice is found", () => {
  assert.deepEqual(split("Something broke.", [word("Something", 0), word("broke", 0.4)], []), [
...
test("radio sentence stays with the voice of most of its words", () => {
  ...
  const words = ["We", "can", "see", "It", "is", "not", "only", "on", "the", "straight", "though"].map((w, i) => word(w, i * 0.3));
  const phrases = [
    { speaker: 2, offsetMilliseconds: 0, durationMilliseconds: 2500 },
    { speaker: 1, offsetMilliseconds: 2600, durationMilliseconds: 1000 },
  ];
  assert.deepEqual(split(text, words, phrases), [{ speaker: 2, text: "We can see. It is not only on the straight though." }]);
});
