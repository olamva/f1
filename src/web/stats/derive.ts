import { addResult, beatsOnCountback, type Standing, type Upcoming } from "../../shared/clinch.ts";
...
export type Who = { id: string; code: string; name: string; team: string; color: string };

export const drivers = (s: Season): Map<string, Who> =>
  new Map(s.drivers.map((d) => [d.id, { id: d.id, code: d.code, name: d.name, team: d.team, color: teamColor(d.team) }]));

export const teams = (s: Season): Map<string, Who> =>
  new Map(s.teams.map((t) => [t.id, { id: t.id, code: t.name, name: t.name, team: t.id, color: teamColor(t.id) }]));
...
const rank = (a: Standing, b: Standing) =>
  b.points - a.points || (beatsOnCountback(a.countback, b.countback) ? -1 : beatsOnCountback(b.countback, a.countback) ? 1 : 0);

function tables(s: Season, key: Key, ids: string[]): Map<number, Standing[]> {
  let table = new Map(ids.map((id) => [id, { id, points: 0, countback: [] } as Standing]));
  const out = new Map<number, Standing[]>();
  const rounds = [...new Set([...s.races, ...s.sprints].map((r) => r.round))].sort((a, b) => a - b);
...
export const driverTables = (s: Season) => tables(s, byDriver, s.drivers.map((d) => d.id));
export const teamTables = (s: Season) => tables(s, byTeam, s.teams.map((t) => t.id));

export const latest = (t: Map<number, Standing[]>): Standing[] => [...t.values()].at(-1) ?? [];
...
export function remaining(s: Season): Upcoming[] {
  ...
  return s.rounds
    ...
    .flatMap((r) => [
      ...(r.sessions.sprint && !sprinted.has(r.round) ? [{ round: r.round, name: `${r.name} Sprint`, kind: "sprint" as const }] : []),
      { round: r.round, name: r.name, kind: "race" as const },
    ]);
}

export function gains(s: Season): { id: string; total: number; races: number }[] {
  ...
  for (const r of s.races.flatMap((x) => x.results)) {
    ...
    out.set(r.driver, { id: r.driver, total: g.total + r.grid - r.position, races: g.races + 1 });
  }
  return [...out.values()].sort((a, b) => b.total - a.total);
}

const best = (q: QualiResult, depth: "q3" | "q2" | "q1") => lapSeconds(q[depth] ?? "");
...
const pairOf = (s: Season, team: string): [string, string] | null => {
  ...
  for (const r of s.races.flatMap((x) => x.results).filter((r) => r.team === team))
...
function duel(s: Season, team: string, a: string, b: string): Duel {
  const d: Duel = { team, a, b, quali: [0, 0], race: [0, 0], points: [0, 0], gap: null };
  const gaps: number[] = [];
  for (const q of s.qualifying) {
    const [x, y] = [q.results.find((r) => r.driver === a), q.results.find((r) => r.driver === b)];
    if (!x || !y) continue;
    d.quali[x.position < y.position ? 0 : 1]++;
    const g = qualiGap(x, y);
    if (g !== null && Math.abs(g) < 3) gaps.push(g);
  }
  for (const r of [...s.races, ...s.sprints].flatMap((x) => [x.results])) {
    const [x, y] = [r.find((e) => e.driver === a), r.find((e) => e.driver === b)];
    d.points[0] += x?.team === team ? x.points : 0;
    d.points[1] += y?.team === team ? y.points : 0;
  }
  for (const r of s.races) {
    const [x, y] = [r.results.find((e) => e.driver === a), r.results.find((e) => e.driver === b)];
    if (x?.position && y?.position) d.race[x.position < y.position ? 0 : 1]++;
  }
  d.gap = median(gaps);
  return d;
}
...
export function seasonRecords(s: Season): SeasonRecord[] {
  return s.drivers.map((d) => {
    const mine = s.races.map((r) => r.results.find((x) => x.driver === d.id)).filter((x) => x !== undefined);
    const points = streaks(mine.map((r) => r.points > 0));
    return {
      id: d.id,
      wins: mine.filter((r) => r.position === 1).length,
      poles: s.qualifying.filter((q) => q.results.find((x) => x.position === 1)?.driver === d.id).length,
      podiums: mine.filter((r) => r.position !== null && r.position <= 3).length,
      dnfs: mine.filter((r) => r.position === null).length,
      pointsStreak: points.current,
      bestPointsStreak: points.best,
      bestPodiumStreak: streaks(mine.map((r) => r.position !== null && r.position <= 3)).best,
    };
  });
}
