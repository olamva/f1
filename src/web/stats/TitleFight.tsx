...
const Pick = ({ label, value, ids, who, onChange }: PickProps) => (
  <label className="flex items-center gap-2 text-sm text-zinc-400">
    {label}
    <select value={value} onChange={(e) => onChange(e.target.value)} className="rounded-md bg-zinc-800 px-2 py-1 text-zinc-100">
      {ids.map((id) => (
...
const pos = (p: number | null) => (p === null ? "P11+ / DNF" : p === OUTSIDE ? "P11+" : `P${p}`);
    ...
    <div>
      ...
      <div className="overflow-x-auto">
        <table className="tabular text-xs">
          <thead>
            <tr>
              <th className="p-1 text-left text-zinc-500">{a} ↓ / {b} →</th>
              {m.theirs.map((t) => (
                <th key={t} className="p-1 text-zinc-400">{pos(t)}</th>
              ))}
            </tr>
          </thead>
          <tbody>
...
const Needs = ({ table, events, focus, rival, who }: MatrixProps) => {
  ...
  return (
    <div>
      <p className="mb-2 text-sm text-zinc-400">
        The points {a} need at {events[0]!.name} to be champion there, for each score of {b}. This assumes that every other team can still score the maximum.
      </p>
      <table className="tabular text-sm">
        <tbody>
          <tr>
            ...
            {needs.map((n) => (
              <td key={n.rival} className="px-1.5 text-center">{n.rival}</td>
            ))}
          </tr>
          <tr>
            <th className="pr-3 text-left text-zinc-400">{a} need</th>
            {needs.map((n) => (
              <td key={n.rival} className={`px-1.5 text-center font-semibold ${n.need === null ? "text-zinc-600" : "text-emerald-400"}`}>
                {n.need ?? "—"}
              </td>
            ))}
          </tr>
...
export const TitleFight = ({ champ, table, events, who }: TitleFightProps) => {
  const alive = table.filter((s) => !isEliminated(table, events, s.id, champ)).map((s) => s.id);
  const [focus, setFocus] = useState(alive[0] ?? "");
  const [picked, setRival] = useState(alive[1] ?? "");
  const rival = picked !== focus ? picked : (alive.find((id) => id !== focus) ?? "");
  const gain = maxGain(events, champ);
  if (!events.length) return <p className="text-zinc-400">The season is over.</p>;
  return (
    <div className="space-y-4">
      <section className="rounded-xl bg-surface p-3">
        <h2 className="mb-1 text-xs font-semibold tracking-wider text-zinc-400 uppercase">Who can still win</h2>
        <p className="mb-2 text-sm text-zinc-500">
          {gain} points remain across {events.length} events. The earliest decider assumes that the driver wins every event and that no rival scores.
        </p>
        <table className="tabular w-full text-sm">
          ...
          <tbody>
            {table.slice(0, 10).map((s) => (
              <tr key={s.id} className={`border-t border-zinc-800 ${alive.includes(s.id) ? "" : "text-zinc-600"}`}>
                <td className="py-1">
                  <span className="flex items-center gap-2">
                    <span className="h-4 w-1 rounded-sm" style={{ background: who.get(s.id)?.color }} />
      ...
      {alive.length > 1 && (
        <section className="space-y-3 rounded-xl bg-surface p-3">
          <h2 className="text-xs font-semibold tracking-wider text-zinc-400 uppercase">What it takes at {events[0]!.name}</h2>
          <div className="flex flex-wrap gap-4">
            <Pick label="Title for" value={focus} ids={alive} who={who} onChange={setFocus} />
            <Pick label="Against" value={rival} ids={alive.filter((id) => id !== focus)} who={who} onChange={setRival} />
          </div>
          {champ === "drivers" ? (
            <Matrix table={table} events={events} focus={focus} rival={rival} who={who} />
          ) : (
            <Needs table={table} events={events} focus={focus} rival={rival} who={who} />
          )}
        </section>
      )}
      <Simulator champ={champ} table={table} events={events} contenders={alive} who={who} />
    </div>
  );
};
